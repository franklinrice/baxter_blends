...
def quaternion_to_exp(rot):
    ...
    try:
    	...
    	theta = 2 * np.arccos(rot.w)
    if theta == 0:
    	omega = 0
    else:
    	try:
    		omega = np.divide(rot[0:3], np.sin(theta/2))
    	except:
    		omega = np.divide([rot.x, rot.y, rot.z], np.sin(theta/2))
    return (omega, theta)
    
def create_rbt(w, theta, trans):
    """
    ...
    R = expm(np.multiply(what,theta))
    p = np.transpose(np.array(trans))
    g = np.vstack((np.column_stack((R, p)), [0,0,0,1]))
    return g
...
def find_omega_theta(R):
    ...
    """
    #YOUR CODE HERE
    theta = np.arccos(np.divide(np.trace(R) - 1,2))
    omega = np.dot(1/(2*np.sin(theta)), ([R[2][1]-R[1][2], R[0][2] - R[2][0], R[1][0]-R[0][1]]))
    return (omega, theta)
    
def find_v(w, theta, trans):
    """
    ...
    what = np.array([[0,-w[2], w[1]],
    				 ...
    				 [-w[1], w[0], 0]])
    if not (w.any()):
    	v = np.divide(trans, np.linalg.norm(trans))
    else:
    	Afirsthalf = np.dot(np.subtract(np.identity(3), expm(np.multiply(what, theta))), what)
    	Asecondhalf = np.dot(np.outer(w,w), theta)
    	A = np.add(Afirsthalf, Asecondhalf)
