...
food_to_tag = {'apple': 'ar_marker_0', 'banana': 'ar_marker_9', 'orange':'ar_marker_0'}
...
class BlenderBaxter:
	def setup(self):
		...
		#self.gripper2 = baxter_gripper.Gripper('right')
		#self.gripper2.calibrate()
		#self.gripper2.close()
		...
		if sys.argv[-1] == "load":
			with open('waypoints.pkl', 'rb') as input1:
			    ...
			    self.off_pos = pickle.load(input1)
		else:
			...
			x = input("press blender off button (1cm above)")
			self.off_pos, self.off_rot = self.secondaryPos()
			...
			with open('waypoints.pkl', 'wb') as output:
				...
				pickle.dump(self.off_pos, output, -1)
	...
	def main(self,args):
		...
		while len(fruits) > 0:
			...
			self.dump() #rotate_to_pose_from_transform(self.dumped_rot)
			self.move_to_pose_from_transform(self.halfway_pos)
			self.move_to_pose_from_transform(self.neutral_pos)
			#self.move_to_pose_from_transform(self.fruit_trans)
			#self.release()
			#self.move_to_pose_from_transform(self.neutral_pos)
	...
	def make_smoothie(self):
		#curr = self.secondaryPos()[0]
		#wp1 = self.move_secondary_arm(np.divide(np.sum([curr, self.dump_pos], axis=0), 2) )

		#self.move_secondary_arm(wp1)
		#self.move_secondary_arm(self.dump_pos)
		wp2 = np.subtract(self.dump_pos, [0,.3,0])
		#self.move_secondary_arm(wp2)
		on_pos = np.subtract(self.action_pos, [0,0,.02])
		#self.move_secondary_arm(on_pos)
		self.move_to_pose_from_transform(wp2)
		self.move_to_pose_from_transform(on_pos)

	def move_to_fruit(self):
		#wp1 = np.subtract(self.neutral_pos, [0,0,.2])
		wp2 = np.add(self.fruit_trans, [.1,0,.2])
	...
	def move_to_blender_through_neutral(self):
		...
		self.move_to_pose_from_transform(self.neutral_pos)
		wp2 = np.sum([self.currentPos()[0], self.dump_pos], axis=0) / 2
		self.move_to_pose_from_transform(wp2)
	...
	# def rotate_to_pose_from_transform(self, rot):
	# 	dest = PoseStamped()
	# 	pos = self.currentPos()
	# 	dest.header.frame_id = 'base'
	# 	dest.pose.position.x =  pos[0][0]
	# 	dest.pose.position.y = pos[0][1]
	# 	dest.pose.position.z = pos[0][2]
		
	# 	#Orientation as a quaternion
	# 	dest.pose.orientation.x = rot[0]
	# 	dest.pose.orientation.y = rot[1]
	# 	dest.pose.orientation.z = rot[2]
	# 	dest.pose.orientation.w = rot[3]
